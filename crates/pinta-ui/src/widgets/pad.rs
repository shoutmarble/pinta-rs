...
pub fn view<'a, Message: 'a>(
    ...
    let header = container(
        row![
            ...
            container(icon::view(IconKind::ChevronDown, 12.0, 12.0, theme.colors.text_muted))
                .width(Length::Fill)
    ...
    let footer = footer_icons.into_iter().fold(
        row!().spacing(theme.spacing.xs).padding([theme.spacing.xs, theme.spacing.sm]),
        |row, icon_kind| {
            row.push(
                container(icon::view(icon_kind, 14.0, 14.0, Color::from_rgb8(0x55, 0x55, 0x5A)))
    ...
    container(
        column![
            container(header)
                .height(Length::Fixed(theme.sizing.dock_header_height as f32)),
            body,
            container(footer)
                .style(move |_| {
                    container::Style::default()
